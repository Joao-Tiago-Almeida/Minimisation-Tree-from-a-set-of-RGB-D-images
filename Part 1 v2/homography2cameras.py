...
def ransac(xy,dim,tresh_num_inliers,tresh_dist_inliers):
    ...
    for i in range(len(xy[0])-3):
        ...
        XY = ( xy[0][i:i+4], xy[1][i:i+4] ) # get four points
...
def getNumInliers( H, xy, dim, tresh_num_inliers ,tresh_dist_inliers, max_inliers ):
    ...
    for i in range(len(xy[0])):
        ...
        point_template_projective = np.array( [ v, u ] ) # it is treated (y, x)
...
final_H = ransac( xy=xy, dim=template.shape[0:2], tresh_num_inliers=0.25, tresh_dist_inliers = 10) 
